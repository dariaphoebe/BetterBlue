...
/// Shortcuts/Siri action to start climate, optionally overriding the
/// target temperature. Unlike the Control Center `StartClimateControlIntent`
/// (preset-only), this exposes a temperature parameter so a Shortcut can
/// set a specific value — including half-degree Celsius. The rest of the
/// climate settings (seats, defrost, duration) come from the vehicle's
/// selected preset.
struct StartClimateIntent: AppIntent {
    static var title: LocalizedStringResource = "Start Climate"
    static var description = IntentDescription(
        "Start climate control, optionally setting the target temperature (Celsius supports half-degree steps)"
    )
    static var openAppWhenRun: Bool = false
    ...
    @Parameter(
        ...
        description: "Optional target temperature. Leave empty to use the selected preset's temperature. Celsius supports half-degree steps."
    )
    var temperature: Measurement<UnitTemperature>?

    init() {}

    init(vehicle: VehicleEntity, temperature: Measurement<UnitTemperature>? = nil) {
        self.vehicle = vehicle
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        let targetVin = vehicle.vin
        let vehicleName = vehicle.displayName
        let override = resolvedTemperature()

        WidgetCommandStatus.record(command: "Start Climate", vin: targetVin)
        WidgetCenter.shared.reloadAllTimelines()

        try await performVehicleActionWithVin(targetVin) { bbVehicle, account, context in
            let selected = bbVehicle.safeClimatePresets.first(where: { $0.isSelected })
                ?? bbVehicle.safeClimatePresets.first
            if let override {
                // Base on the selected preset's settings, override the temp.
                var options = selected?.climateOptions
                    ?? ClimateOptions(preferredUnits: AppSettings.liveTemperatureUnit())
                options.temperature = override
                try await account.startClimate(
                    bbVehicle,
                    options: options,
                    modelContext: context,
                    presetName: selected?.name ?? "Climate",
                    presetIcon: selected?.iconName ?? "fan"
                )
            } else {
                // No override — let startClimate resolve the selected preset.
    ...
    /// Map the Shortcuts measurement to a `Temperature`, snapping Celsius
    /// to the 0.5°C grid and Fahrenheit to whole degrees.
    private func resolvedTemperature() -> Temperature? {
        guard let temperature else { return nil }
        if temperature.unit == UnitTemperature.fahrenheit {
            return Temperature(value: temperature.value.rounded(), units: .fahrenheit)
        }
        let celsius = temperature.converted(to: .celsius).value
        return Temperature(value: Temperature.snapToHalfDegreeCelsius(celsius), units: .celsius)
    }
}
